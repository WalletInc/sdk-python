# coding: utf-8

"""
    wallet-api

    Wallet Inc. API reference.  **Spec version 2.1.604**, built 2026-07-06T14:50:23.179Z

    The version of the OpenAPI document: 2.1.604
    Contact: development@wallet.inc
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from wallet.models.payment_design import PaymentDesign

class TestPaymentDesign(unittest.TestCase):
    """PaymentDesign unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> PaymentDesign:
        """Test PaymentDesign
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `PaymentDesign`
        """
        model = PaymentDesign()
        if include_optional:
            return PaymentDesign(
                border_color = '#f0f0f0',
                border_style_type = Dashed,
                border_size = '4px',
                border_radius = 4,
                font_color = '#777777',
                font_type = 'Arial',
                abbreviation = 'ABBR',
                acronym = 'AR',
                icon = 'fa-anchor',
                design_name = 'Thanksgiving Design',
                display_name = 'Thanksgiving Coupon',
                background_image_url = 'https://example.com/background-image',
                company_logo_url = 'https://example.com/company-logo',
                employee_id = 'ueJDL982Hs',
                id = jdu37eua98,
                merchant_id = 'ueJDL982Hs',
                created_at = '2016-06-22 19:10:25-07',
                updated_at = '2016-06-22 19:10:25-07',
                is_active = True
            )
        else:
            return PaymentDesign(
                border_color = '#f0f0f0',
                border_style_type = Dashed,
                border_size = '4px',
                border_radius = 4,
                font_color = '#777777',
                font_type = 'Arial',
                abbreviation = 'ABBR',
                acronym = 'AR',
                icon = 'fa-anchor',
                design_name = 'Thanksgiving Design',
                display_name = 'Thanksgiving Coupon',
                employee_id = 'ueJDL982Hs',
                id = jdu37eua98,
                merchant_id = 'ueJDL982Hs',
                created_at = '2016-06-22 19:10:25-07',
                updated_at = '2016-06-22 19:10:25-07',
                is_active = True,
        )
        """

    def testPaymentDesign(self):
        """Test PaymentDesign"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
