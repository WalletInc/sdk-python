...
class WTAdvertisementCreditScan(BaseModel):
    ...
    date_time_redeemed: Optional[datetime] = Field(alias="dateTimeRedeemed")
    date_time_refunded: Optional[datetime] = Field(alias="dateTimeRefunded")
    ...
    @field_validator('advertisement_credit_id')
    def advertisement_credit_id_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if not re.match(r"^[a-zA-Z0-9]+$", value):
            raise ValueError(r"must validate the regular expression /^[a-zA-Z0-9]+$/")
        return value
